# -*- coding: utf-8 -*-
"""
Created on 2025-07-04 (Fri) 23:04:27

バッチサイズが推定性能に及ぼす影響の把握

@author: I.Azuma
"""
# %%
BASE_DIR = '/workspace/cluster/HDD/azuma/Azuma_DDI'
...
import sys
sys.path.append(BASE_DIR +'/github/XAI-DDI')

import os
os.chdir(f'{BASE_DIR}/github/XAI-DDI/baseline/DSN_DDI')
...
parser.add_argument('--pkl_name', type=str, default=f'{BASE_DIR}/workspace/Baseline/DSN_DDI/checkpoints/251007/inductive_drugbank.pkl')
...
# %% Dataset
df_ddi_train = pd.read_csv(BASE_DIR + '/github/Drug-Interaction-Research/drugbank_test/inductive_data/fold1/train.csv')
df_ddi_s1 = pd.read_csv(BASE_DIR + '/github/Drug-Interaction-Research/drugbank_test/inductive_data/fold1/s1.csv')
df_ddi_s2 = pd.read_csv(BASE_DIR + '/github/Drug-Interaction-Research/drugbank_test/inductive_data/fold1/s2.csv')
...
# %% Test phase
test_model = torch.load(f'{BASE_DIR}/workspace/Baseline/DSN_DDI/checkpoints/251007/inductive_drugbank.pkl')
...
with torch.no_grad():
    for batch in s1_data_loader:
        test_model.eval()
        p_score, n_score, probas_pred, ground_truth = do_compute(batch, device, model=test_model)
        s1_probas_pred.append(probas_pred)
        s1_ground_truth.append(ground_truth)
...
"""
 s1 with 5994, and s2 with 56201

============================== Best Result ==============================
s1_acc: 0.7020, s1_roc: 0.7951, s1_f1: 0.6472, s1_precision: 0.7933,s1_recall: 0.5465,s1_int_ap: 0.7927,s1_ap: 0.7928
s2_acc: 0.7812, s2_roc: 0.8708, s2_f1: 0.7643, s2_precision: 0.8281,s2_recall: 0.7096,s2_int_ap: 0.8659,s2_ap: 0.8659
"""
